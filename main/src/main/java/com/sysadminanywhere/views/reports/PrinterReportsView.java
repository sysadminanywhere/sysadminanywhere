...
public class PrinterReportsView extends Div implements HasDynamicTitle {
        ...
        listBox.addValueChangeListener(event -> {
            if (event.getValue() != null) {
                event.getSource().getUI().ifPresent(ui ->
                        ui.navigate("reports/report?entry=printers&id=" + event.getValue().getId()));
            }
        });
        add(listBox);
    }
    ...
    private void addReports() {
        try (InputStream inputStream = new ClassPathResource("reports/printers.json").getInputStream();
            ...
            throw new RuntimeException("Failed to load reports from printers.json", e);
        }
    }
    ...
    public String getPageTitle() {
        return getMessage("printer_reports_view.title");
    }
}
