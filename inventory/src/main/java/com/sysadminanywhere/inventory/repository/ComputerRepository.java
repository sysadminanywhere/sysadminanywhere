...
public interface ComputerRepository extends JpaRepository<Computer, Long>, JpaSpecificationExecutor<Computer> {
    ...
    @Query("SELECT new com.sysadminanywhere.common.inventory.model.ComputerItem(c.id, c.name) FROM Computer c JOIN c.installations i WHERE i.software.id = :softwareId and c.name LIKE :name")
