...
public interface ComputerRepository extends JpaRepository<Computer, Long>, JpaSpecificationExecutor<Computer> {
    ...
    @Query("SELECT new com.sysadminanywhere.common.inventory.model.ComputerItem(c.id, c.name, ch.checkingDate) FROM Computer c JOIN c.computerHardwares ch WHERE ch.hardwareModel.hardwareType.id = :hardwareTypeId and c.name LIKE :name")
    Page<ComputerItem> getComputersWithHardware(@Param("hardwareTypeId") Long hardwareTypeId, @Param("name") String name, Pageable pageable);
