...
public interface ComputerRepository extends JpaRepository<Computer, Long>, JpaSpecificationExecutor<Computer> {
    ...
    // select c."name", ch.checking_date from computers c inner join computer_hardware ch on c.id = ch.computer_id where ch.hardware_id = 1
    @Query("SELECT new com.sysadminanywhere.common.inventory.model.ComputerItem(c.id, c.name, ch.checkingDate) FROM Computer c JOIN c.computerHardwares ch WHERE ch.hardware.id = :hardwareId and c.name LIKE :name")
    Page<ComputerItem> getComputersWithHardware(@Param("hardwareId") Long hardwareId, @Param("name") String name, Pageable pageable);
